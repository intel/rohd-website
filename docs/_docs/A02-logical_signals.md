...
The fundamental signal building block in ROHD is called [`Logic`]({{ site.baseurl }}api-rohd/rohd/Logic-class.html).
...
The value of a `Logic` is of type [`LogicValue`]({{ site.baseurl }}api-rohd/rohd/LogicValue-class.html), with pre-defined constant bit values `x`, `z`, `one`, and `zero`.  `LogicValue` has a number of built-in logical operations `(like &, |, ^, +, -, etc.)`.
